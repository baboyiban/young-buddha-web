 ...
 */
function _mapRowToPaymentRequest(row: SheetsRow, name?: string): PaymentRequest {
  const cells = row.c ?? [];
 ...
 */
async function _processPaymentRows(rows: SheetsRow[], skipNameLookup: boolean): Promise<PaymentRequest[]> {
  ...
  // 이름 조회가 필요한 경우
  const emails = Array.from(new Set(validRows.map(row => row.c?.[1]?.v as string).filter(Boolean)));

  const emailToNameMap = new Map<string, string>();
  if (emails.length > 0) {
    const names = await Promise.all(emails.map(email => getUserNameByEmail(email)));
 ...
 * @returns 조회된 데이터 (문자열)
 */
async function _getUserDataByEmail(email: string, columnToSelect: 'B' | 'C'): Promise<string | null> {
  ...
  const data = (await sheetsRead(USER_SHEET.spreadsheetId, USER_SHEET.sheetName, query)) as SheetsData;
...
function buildWhereConditions(userEmail: string | null, statusFilter: string, typeFilter?: string): string[] {
  const conditions: string[] = [];

  if (userEmail) {
    conditions.push(`${PAYMENT_COLUMNS.EMAIL} = '${escapeSheetQueryString(userEmail)}'`);
  }
  if (statusFilter !== "전체") {
    conditions.push(`${PAYMENT_COLUMNS.STATUS} = '${escapeSheetQueryString(statusFilter)}'`);
  }
  if (typeFilter && typeFilter !== "전체") {
    conditions.push(`${PAYMENT_COLUMNS.TYPE} = '${escapeSheetQueryString(typeFilter)}'`);
  }

  return conditions;
}

function buildDataQuery(conditions: string[], sortOrder: string, limit: number, offset: number): string {
...
export async function fetchFilteredPayments(
  ...
  const conditions = buildWhereConditions(userEmail || null, statusFilter, typeFilter);
  const query = buildDataQuery(conditions, sortOrder, limit, offset);

  const data = (await sheetsRead(PAYMENT_SHEET.spreadsheetId, PAYMENT_SHEET.sheetName, query)) as SheetsData;
...
export async function getTotalPaymentCount(
  ...
  try {
    const conditions = buildWhereConditions(userEmail, statusFilter, typeFilter);
    const countQuery = buildCountQuery(conditions);

    const data = (await sheetsRead(PAYMENT_SHEET.spreadsheetId, PAYMENT_SHEET.sheetName, countQuery)) as SheetsData;
