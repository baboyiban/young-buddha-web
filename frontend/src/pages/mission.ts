...
export async function loadMissionData(): Promise<void> {
  ...
  try {
    ...
    resultDiv.innerHTML = `오류 발생: ${error instanceof Error ? error.message : String(error)}`;
  } finally {
    isLoading = false;
  }
}
...
function createMissionHtml(items: string[]): string {
  return `
    <div class="flex flex-col items-center justify-center-safe *:not-last:mb-[1rem] *:text-center *:*:not-last:mb-[0.25rem]">
      ...
      <div><div>🍚 발우공양 당번</div><div>${[items[2], items[3]].filter(Boolean).join(", ")}</div></div>
      <div><div>🤲 발우공양 바라지</div><div>${[items[4], items[5]].filter(Boolean).join(", ")}</div></div>
      <div><div>🧼 아침 설거지</div><div>${[items[6], items[7], items[8]].filter(Boolean).join(", ")}</div></div>
      <div><div>🧺 걸레빨기</div><div>${[
        ...
        .filter(Boolean)
        .join(", ")}</div></div>
      <div><div>🌞 사시예불전 방석깔기</div><div>${[items[12] || "상근자 전원", items[13]].filter(Boolean).join(", ")}</div></div>
      <div><div>🍛 저녁공양 당번</div><div>${[items[14], items[15], items[16]].filter(Boolean).join(", ")}</div></div>
